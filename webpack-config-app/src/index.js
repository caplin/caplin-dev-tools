...
for (const packageDir of readdirSync(join('../../packages'))) {
	packageRoots.push(join(__dirname, `node_modules/${packageDir}/src`));

	try {
		accessSync(join(__dirname, `node_modules/${packageDir}/compiler.json`), F_OK);
	} catch (e) {
		babelLoaderExclude.push(join(__dirname, `node_modules/${packageDir}/`));
	}
}

const variant = parseArgs(process.argv.slice(2)).variant || 'caplin';
const appEntryPoint = join(__dirname, `entry-${variant}.js`);
const buildOutputDir = join(__dirname, 'dist', 'public');
