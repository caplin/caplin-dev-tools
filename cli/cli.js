...
var runCommand = function(commandAndOptionList) {
  ...
  var chosenCommandOptions = commandAndOptionList.splice(
    1,
  ...
  return {
    ...
    message: (
      commandName
        ? 'Command "' +
            commandName +
            '" not found, what command do you want to run:'
        : "What command do you want to run:"
    ),
    choices: orderedCommands.map(function(command) {
      return command.name;
    })
  };
};
