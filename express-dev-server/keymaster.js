...
function getTimeStamp() {
  ...
  // if an offset time has been provided, the timestamp will be modified.
  // e.g. if the client is based in New York and the keymaster server is in London,
  // an offset of 5 would be used to add 5 hours to the client timestamp
  if (localTimeOffset) {
    const timeWithOffset =
      now.getTime() + localTimeOffset * HOURS_IN_MILLISECONDS;
...
module.exports = (
  application,
  { keyDirectory, customerID = "" },
  defaultLocalTimeOffset = 0
) => {
  defaultCustomerID = customerID;
  localTimeOffset = defaultLocalTimeOffset;
